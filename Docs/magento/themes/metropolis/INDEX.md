...
Metropolis is a responsive Magento theme. That means that there is no need for an additional mobile version of your store, as the template automatically adapts to the viewing device, such as a smartphone, tablet or desktop, based on the resolution of the screen.
...
Installing the Metropolis theme is quick and easy to do. There are numerous ways to do it, but we will outline the most common method(s) in this tutorial. We will start with the process you will likely want to go through if you are installing Metropolis on an existing Magento install.
...
[download]: http://www.rockettheme.com/magento/themes/metropolis
